...
if __name__ == "__main__":
    ...
    for i in range(len(input)):
        for j in range(len(input)):
            if i == j: 
                continue
            result = Node.parse_list(input[i]) + Node.parse_list(input[j])
            max_value = max(max_value, result.magnitude())
    
    print(f"The largest magnitude of any sum of two different snailfish numbers from the homework assignment is: {max_value}")